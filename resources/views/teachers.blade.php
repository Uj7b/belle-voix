...
<div class="main">
  ...
  <div class="content">
    <div class="page-header">
      <div>
        <h1>Teachers</h1>
        <div class="sub" id="enrollCount">{{ $teacherCount }} enrolled students</div>
      </div>
...
function enrollTeacher(){
  ...
  if(!name||!email||!cls||!teacher||!gender||!dob){ showToast('⚠️ Please fill in all required fields.'); return; }
  const id='STU-'+String(nextNum).padStart(3,'0');
  students.push({id,name,email,cls,teacher,gender,dob,status});
  nextNum++;
  document.getElementById('enrollCount').textContent=students.length.toLocaleString()+' enrolled students';
  ['f-name','f-email','f-class','f-teacher','f-gender','f-dob'].forEach(i=>document.getElementById(i).value='');
